    ...
    public class CrearDireccion
    {
        ...
        public class Manejador : IRequestHandler<Ejecutar, string>
        {
            ...
            public async Task<string> Handle(Ejecutar request, CancellationToken cancellationToken)
            {
                ...
                var existe = db.PersonaDireccion.Where(x => x.idpersona == request.idpersona && x.direccion == request.direccion).Any();
                if (existe)
                    throw new ManageException(System.Net.HttpStatusCode.BadRequest, new { mensaje = "Ya se ha registrado la direccion" });
                await db.AddAsync(new PersonaDireccion { idpersona = request.idpersona, direccion = request.direccion });
